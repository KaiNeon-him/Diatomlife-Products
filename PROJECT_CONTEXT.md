...
- **Product seeding:** The static 19 products from `src/app/data/products.ts` need to be inserted into Supabase. Multiple seed script attempts failed due to schema mismatches (`category` vs `category_id`, `scientific_info` vs `scientific_why`, CTE parsing errors in Supabase editor). Latest version: `supabase/seed_products_v2.sql` generated by `scripts/gen_seed.cjs`. **User has not yet confirmed a successful run.** If it still fails, regenerate with `node scripts/gen_seed.cjs` or write a fresh simple INSERT-only script matching live schema exactly.
...
| `supabase/seed_products_v2.sql` | Latest product seed attempt (verify/run) |
| `scripts/gen_seed.cjs` | Generator that converts products.ts → seed SQL |
...
1. Confirm whether `supabase/seed_products_v2.sql` ran successfully. If not, diagnose from actual live schema (`select column_name from information_schema.columns where table_name='products';`) and rewrite seed accordingly.
